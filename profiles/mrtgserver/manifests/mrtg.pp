...
cron {'indexmaker_sensores':
    user    => $owner,
    command => '/usr/bin/indexmaker --columns=2 --addhead="<H1 align= "center" > Multi Router Traffic Grapher <H1>" --title="Temperatura y Humedad DATACENTER-UPR" /etc/mrtg/mrtg.sensor1.cfg 2> /dev/null& /etc/mrtg/mrtg.sensor2.cfg 2> /dev/null& > /var/www/mrtg/sensores/index.html',
    minute  => '*/1'
  }
}
